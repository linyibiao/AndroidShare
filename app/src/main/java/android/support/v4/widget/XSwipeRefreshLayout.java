 ...
 * </p>
 */
public class XSwipeRefreshLayout extends ViewGroup implements NestedScrollingParent,
     ...
     */
    public boolean canChildScrollUp(DIRECTION direction) {
        if (direction == DIRECTION.TOP) {
            ...
            if (android.os.Build.VERSION.SDK_INT < 14) {
                if (mTarget instanceof AbsListView) {
                    final AbsListView absListView = (AbsListView) mTarget;
                    return absListView.getChildCount() > 0
                            && (absListView.getLastVisiblePosition() < absListView.getCount() - 1 || absListView.getChildAt(absListView.getLastVisiblePosition() - absListView.getFirstVisiblePosition())
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mIsBeingDragged) {
                    ...
                    if ((yDiff > mTouchSlop && mode != MODE.BOTTOM_ONLY && !canChildScrollUp(DIRECTION.TOP)) || (yDiff < -mTouchSlop && mode != MODE.TOP_ONLY && !canChildScrollUp(DIRECTION.BOTTOM))) {
                        if (yDiff > mTouchSlop) {
...
//                            mInitialMotionY = mInitialDownY - mTouchSlop;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || mReturningToStart || (canChildScrollUp(DIRECTION.TOP) && canChildScrollUp(DIRECTION.BOTTOM)) || mNestedScrollInProgress) {
            // Fail fast if we're not in a state where a swipe is possible
            return false;
        }
    ...
    private MODE mode = MODE.TOP_ONLY;
